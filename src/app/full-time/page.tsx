                                            ...
                                            <div key={m.id} className="bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden">
                                                ...
                                                {expandedMeeting === m.id && summaries[m.id] && (
                                                    <div className="border-t border-slate-100 bg-slate-50 p-5 space-y-4 text-sm">
                                                        {/* 標題 */}
                                                        <h4 className="font-black text-blue-900 text-base">{summaries[m.id].title}</h4>

                                                        {/* 結論 */}
                                                        {summaries[m.id].conclusions?.length > 0 && (
                                                            <div>
                                                                <p className="font-bold text-slate-700 mb-1">📌 會議結論</p>
                                                                <ul className="list-disc list-inside space-y-1 text-slate-600">
                                                                    {summaries[m.id].conclusions.map((c: string, i: number) => (
                                                                        <li key={i}>{c}</li>
                                                                    ))}
                                                                </ul>
                                                            </div>
                                                        ...
                                                        {/* 工作進度 */}
                                                        {summaries[m.id].work_updates?.length > 0 && (
                                                            <div>
                                                                <p className="font-bold text-slate-700 mb-1">🗂 工作進度</p>
                                                                <div className="space-y-1">
                                                                    {summaries[m.id].work_updates.map((w: any, i: number) => (
                                                                        <div key={i} className="flex gap-2 text-slate-600">
                                                                            <span className="font-semibold text-slate-800 whitespace-nowrap">{w.member}：</span>
                                                                            <span>{w.update}</span>
                                                                        </div>
                                                                    ))}
                                                                </div>
                                                        ...
                                                        {/* 關鍵引用 */}
                                                        {summaries[m.id].citations?.length > 0 && (
                                                            <div>
                                                                <p className="font-bold text-slate-700 mb-1">💬 關鍵引用</p>
                                                                <div className="space-y-2">
                                                                    {summaries[m.id].citations.map((c: any, i: number) => (
                                                                        <div key={i} className="bg-white border-l-4 border-blue-900 pl-3 py-1 rounded-r">
                                                                            <p className="text-slate-800 italic">「{c.text}」</p>
                                                                            <p className="text-xs text-slate-500 mt-0.5">{c.context}</p>
                                                                        </div>
                                                                    ))}
                                                                </div>
                                                            </div>
